...
def _soft_validate_live(cfg, bucket_by_t, da_by_t, *, label_by_t: dict | None = None, preview_by_t: dict | None = None):
    """
    Live checks: bucket selected + DA in range.
    Do NOT enforce bucket ordering during live editing.

    IMPORTANT: Messages must not leak system IDs (t#). Use display-position labels instead.
    """
    reasons: List[str] = []
    label_by_t = label_by_t or {}
    preview_by_t = preview_by_t or {}

    def _label(t: str) -> str:
        return str(label_by_t.get(t) or f"Translation {t}")

    def _preview(t: str) -> str:
        p = str(preview_by_t.get(t) or "").strip()
        if not p:
            return ""
        p = " ".join(p.split())
    ...
    for t, b in bucket_by_t.items():
        if b is None:
            pv = _preview(t)
            reasons.append(f"Missing bucket for {_label(t)}." + (f"  
↳ {pv}" if pv else ""))
    ...
    for t, d in da_by_t.items():
        if d is None:
            pv = _preview(t)
            reasons.append(f"Missing DA for {_label(t)}." + (f"  
↳ {pv}" if pv else ""))
            continue
        if int(d) < cfg.da_min or int(d) > cfg.da_max:
            pv = _preview(t)
            reasons.append(f"DA for {_label(t)} out of range [{cfg.da_min}, {cfg.da_max}]." + (f"  
↳ {pv}" if pv else ""))
...
def _download_button(label: str):
    # For downloads, sync UI state into the XLSX so the file reflects what the rater sees,
    # but do NOT clear committed_at/row hashes. Downloading should never make a
    # previously-completed worksheet look incomplete.
...
# Build non-leaking labels/previews for live validation (use display positions, not t# system IDs)
label_by_t_live: Dict[str, str] = {}
preview_by_t_live: Dict[str, str] = {}
try:
    for _pos, _tcol, _t_idx, _txt, *_rest in pos_to_current:
        label_by_t_live[str(_tcol)] = f"Translation {_pos}"
        preview_by_t_live[str(_tcol)] = str(_txt or "")
except Exception:
    label_by_t_live = {}
    preview_by_t_live = {}
live_ok, live_reasons = _soft_validate_live(cfg, bucket_by_t_live, da_by_t_live, label_by_t=label_by_t_live, preview_by_t=preview_by_t_live)
